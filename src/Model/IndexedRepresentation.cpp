...
IndexedRepresentation::IndexedRepresentation(const IndexedRepresentation& other)
{
  for(unsigned int i = 0; i < other.m_indices.size(); i ++)  
    m_indices.push_back(other.m_indices[i]);
  for(unsigned int i = 0; i < other.m_positions.size(); i ++)  
  {
    glm::vec3 t= other.m_positions[i];
    m_positions.push_back(t);
  }
  for(unsigned int i = 0; i < other.m_texCoords.size(); i ++)  
    m_texCoords.push_back(other.m_texCoords[i]);
  for(unsigned int i = 0; i < other.m_normals.size(); i ++)  
    m_normals.push_back(other.m_normals[i]);
  for(unsigned int i = 0; i < other.m_bones.size(); i ++)  
    m_bones.push_back(other.m_bones[i]);  
}

IndexedRepresentation & IndexedRepresentation::operator=(const IndexedRepresentation& other)
{
  for(unsigned int i = 0; i < other.m_indices.size(); i ++)  
    m_indices.push_back(other.m_indices[i]);
  for(unsigned int i = 0; i < other.m_positions.size(); i ++)  
