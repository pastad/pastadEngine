...
void passSSAO()
{
 ...
 // if( texture(Tex7, TexCoord).xyz != vec3(0,0,0) )
  ...
  vec3 noise = vec3( texture(Tex8, TexCoord* noiseScale) );

  vec3 tan = vec3( texture( Tex6, TexCoord ) ) ;// normalize(noise - norm* dot(noise, norm));
  vec3 bitan = vec3( texture( Tex7, TexCoord ) ) ;//cross(norm, tan);
  ...
  for( int x=0; x< 64; x++)
  {
	vec3 sam = trans * SSAOSamples[x]; // TODO :nope fix me
	float radius = 0.1;
	sam = fp.xyz + sam *radius; // 1.0 radius

	vec4 off = CameraView * vec4(sam,1.0);
	off =  CameraProjection * off;
	off.xyz /= off.w;
	off.xyz = off.xyz *0.5 +0.5;

	float depth = texture(Tex4, off.xy ).z; // the depth from the camera
	float sam_depth = (CameraProjection * CameraView * vec4(sam,1.0 )).z; // sample point to CameraProjection and get depth
   ...
   // else
     // depth  = distance (texture(Tex1, off.xy).xyz, CameraPosition);
	  ...
	  float range = smoothstep(0.0,1.0, radius/ abs(sam_depth-depth) );
	  occ += ( depth >= sam_depth +0.001 ? 1.0 : 0.0 )  * range;	
	  
  }

  float depth = texture(Tex4, TexCoord ).z;
  depth  = distance (pos, CameraPosition);
  occ = 1.0 - (occ /64.0);
