...
layout(location = 6 ) out vec3 TangentData;
layout(location = 7 ) out vec3 BitangentData;
...
vec4 calcDirectionalLight(int idx, Material mat, vec3 pos, vec3 normal)
{
  ...
  vec3 ambient =  l.Base.AmbientColor *0.5 *  texture(Tex3, TexCoord).xyz   ;//l.Base.AmbientColor * mat.AmbientColor;
  if(EnableSSAO == 1)// && (texture(Tex7, TexCoord).xyz == vec3(0,0,0) ))
    ambient *=  ( texture(Tex8, TexCoord).x ) ;
  ...
  return vec4(diffuse* shadowFactor * l.Base.Intensity +ambient* l.Base.Intensity *0.1 + specular* shadowFactor * l.Base.Intensity,1) ; // EDIT DEBUG
}
...
{  
  ...
  vec3 ambient =  l.Base.AmbientColor  *0.5 *  texture(Tex3, TexCoord).xyz  ;//l.Base.AmbientColor * mat.AmbientColor;
  if(EnableSSAO == 1)// && (texture(Tex7, TexCoord).xyz == vec3(0,0,0) ) )
   ambient =   texture(Tex8, TexCoord).x  *  texture(Tex3, TexCoord).xyz  ;
  ...
  return vec4( (diffuse* l.Base.Intensity* shadowFactor + ambient* l.Base.Intensity *0.1 + specular* shadowFactor * l.Base.Intensity   ) * attenuation,1)  ;
}
...
vec4 calcSpotLight(int idx, Material mat, vec3 pos, vec3 normal)
{
  ...
  vec3 ambient =  l.Pointlight.Base.AmbientColor * 0.5 *  texture(Tex3, TexCoord).xyz ;//l.Base.AmbientColor * mat.AmbientColor;
...
void pass1()
{
  ...
  if(NormalMapActive == 1 )
  {
    ...
    NormalData = normalize(Normal) +  TBN * normalize(NormalData * 2.0 - 1.0  );   
  ...
  if(BumpMapActive == 1 )
  {
  //  PositionData = Position + NormalData * texture( Tex7, TexCoord ).x;
  }
